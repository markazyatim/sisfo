...
// ----------------------------------------------------
// FUNGSI UTAMA SINKRONISASI DENGAN SUPER CACHE
// ----------------------------------------------------
let isAppSyncing = false; // ANTI-TABRAKAN SINKRONISASI

async function loadAppData(isInitialLoad = false) {
    if (!currentUser) return;
    if (isAppSyncing) return; // Jika sedang proses sync, batalkan pemanggilan ganda
    
    isAppSyncing = true;
    const originalTitle = document.getElementById('page-title').textContent;
    document.getElementById('page-title').textContent = "Menyinkronkan Data...";

    const cacheKey = `portal_appData_${currentUser.username}`;
    const cachedDataStr = localStorage.getItem(cacheKey);
    
    if (cachedDataStr) {
        try {
            const cachedDb = JSON.parse(cachedDataStr);
            applyDataToApp(cachedDb);
            updateUserInterface();
            if (isInitialLoad) {
            ...
            console.warn("Cache rusak, akan mengunduh ulang dari server.");
        }
    }
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            if (isInitialLoad && !cachedDataStr) {
                 if(typeof hideGlobalLoading === "function") hideGlobalLoading();
                 navigate('dashboard');
        ...
        console.warn("Gagal terhubung ke Database. Mode Offline (Cache) aktif.", error);
    } finally {
        document.getElementById('page-title').textContent = originalTitle;
        isAppSyncing = false; // Buka kembali gerbang sinkronisasi
    }
}
...
// ==================================================
function forceSyncSystem() {
    if (!confirm("Tindakan ini akan memuat ulang aplikasi untuk menarik pembaruan kode sistem dan data terbaru dari database. Lanjutkan?")) return;
    
    if(typeof showGlobalLoading === "function") showGlobalLoading("Menarik pembaruan sistem & data terbaru...");
    ...
    // 3. Paksa Hard Reload Browser (Bypass Cache JS/CSS)
    setTimeout(() => {
        // Menambahkan parameter waktu acak ke URL agar browser DIPAKSA mengunduh ulang file dari server
